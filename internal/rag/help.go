...
func ShowHelp(maxTokensPerChunk, chunkOverlapPercent, maxContextTokens int) {
	...
	fmt.Println("  -mcp                       Run as MCP server over STDIO transport")
	fmt.Println("  -sse                       Run as MCP server over HTTP with SSE transport")
	fmt.Println("  -sse-addr <addr>           Address for SSE server (default: :8080)")
